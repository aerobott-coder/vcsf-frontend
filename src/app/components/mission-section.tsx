...
export function MissionSection() {
    ...
    return (
        <section id="mission" className="py-14 sm:py-24 bg-white relative overflow-hidden">
            {/* Decorative Background Elements */}
            <div className="absolute top-0 right-0 w-48 sm:w-64 h-48 sm:h-64 bg-[#D4AF37]/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl"></div>
            <div className="absolute bottom-0 left-0 w-64 sm:w-96 h-64 sm:h-96 bg-[#0F2C59]/5 rounded-full translate-y-1/2 -translate-x-1/2 blur-3xl"></div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 justify-items-center">
                    {missionPoints.map((point, index) => (
                        <motion.div
                            ...
                            className={`w-full bg-gray-50 p-5 sm:p-8 rounded-2xl border border-gray-100 hover:border-[#D4AF37] hover:shadow-xl transition-all duration-300 group ${
                                index === 4 ? "sm:col-span-2 lg:col-span-1" : ""
                            }`}
                        >
                            <div className="w-12 sm:w-16 h-12 sm:h-16 bg-white rounded-xl flex items-center justify-center mb-4 sm:mb-6 shadow-sm group-hover:bg-[#0F2C59] transition-colors duration-300">
                                <div className="group-hover:text-white transition-colors duration-300">
                                    {point.icon}
                                </div>
                            </div>
                            <h3 className="text-[#0F2C59] text-lg sm:text-xl font-bold mb-2 sm:mb-3 group-hover:text-[#D4AF37] transition-colors duration-300">
                                {point.title}
                            </h3>
