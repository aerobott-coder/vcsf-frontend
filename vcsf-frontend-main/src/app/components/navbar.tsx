...
export function Navbar({ forceSolid = false }: { forceSolid?: boolean }) {
    ...
    const isSolid = scrolled || forceSolid || isBlogPage;
    ...
    return (
        <nav
            className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isSolid ? "bg-white shadow-lg py-2 sm:py-3" : "bg-transparent py-3 sm:py-5"
                }`}
        >
            <div className="container mx-auto px-4 sm:px-6">
                <div className="flex justify-between items-center">
                    ...
                    <a href="/#home" className="flex items-center space-x-2 sm:space-x-3 group">
                        ...
                        <span className={`text-base sm:text-xl font-bold tracking-tight whitespace-nowrap transition-colors duration-300 ${isSolid ? "text-[#0F2C59]" : "text-white"
                            }`}>
                    ...
                    <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
                        {navLinks.map((link) => (
                            <a
                                key={link.name}
                                href={link.path}
                                className={`text-xs xl:text-sm font-bold uppercase tracking-wider transition-all duration-300 hover:text-[#D4AF37] relative group ${isSolid ? "text-[#0F2C59]" : "text-white"
                                    }`}
                            >
                                {link.name}
                    ...
                    <div className="lg:hidden">
                        <button
                            onClick={toggleMenu}
                            className={`p-2 rounded-lg transition-colors ${isSolid ? "text-[#0F2C59] hover:bg-gray-100" : "text-white hover:bg-white/10"
                                }`}
